...
def read_data(company_code,
    ...
    assert len(high_frequency_volatilities) > 0

    if shuffle:
        high_frequency_volatilities = high_frequency_volatilities.sample(frac=1)

    column_number = len(high_frequency_volatilities.columns) - 1
    units = high_frequency_volatilities.loc[:, high_frequency_volatilities.columns != 'label']
    ...
    train_dates = dates[high_frequency_volatilities['date'] < test_start_date]['date']
    test_units = units[high_frequency_volatilities['date'] >= test_start_date]
    test_labels = labels[high_frequency_volatilities['date'] >= test_start_date]['label']
    test_dates = dates[high_frequency_volatilities['date'] >= test_start_date]['date']
